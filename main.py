...
import importlib
app = Flask(__name__)
app.config['SECRET_KEY'] = config.SECERT_KEY							# Sets the sessions secret_key. Randomly generated every time the app is restarted.
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=30)		# Sets the cookie / session lifetime to 30 minutes, meaning every 30 minutes its cleared.
