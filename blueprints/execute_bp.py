...
def execute():
    ...
    if payment.execute({'payer_id': request.form['payerID']}):
        ...
        if item == "1000MB Storage":
            ...
            usersCollection.update_one(filter, newValues)
        if item == "1 Server Slots":
            currentServerSlot = data.get('server_limit')
            currentServerSlot += 1
            filter = {'email': email}
            newValues = { "$set": { 'server_limit': currentServerSlot}}
            usersCollection.update_one(filter, newValues)
        flash("Success! Your purchase was executed successfully.")
